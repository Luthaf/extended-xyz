...
with open("extended-xyz.lark") as fd:
    grammar = fd.read()
...
class ExtendedXyzAsDict(lark.Visitor):
    ...
    def generic_frame_property(self, value):
        ...
        self._exyz_dict["info"][key] = value
    ...
    def atom_properties_list(self, value):
        ...
        self._exyz_dict["info"]["Properties"] = properties

    def atom_line(self, value):
        atom = []
        for data in value.children:
            atom.append(data.children[0])
        self._exyz_dict["atoms"].append(atom)


with open(sys.argv[1]) as fd:
    data = fd.read()

parser = lark.Lark(grammar, parser="earley", start="file")
